# coding: utf-8
'''
Ellvator is released under the AGPL-3.0 Licence.
...
def search():
  ...
  if re.match('i:', query):
    ...
    top_i = 1
  if page == 1:
    im_word = ['screen', 'photo', 'panorama', 'logo', 'icon', 'wallpaper', 'image']
    if 'news' in query:
      top_nw = 1
    for i in im_word:
      if i in query.lower():
  ...
  if top_iw == 1:
    top_i = 1
  get_url = "http://localhost:50001/search?" + up.urlencode({'q': query, 'p': page})
  if page == 1:
    page_back = 1
    page_next = 2
  elif page == 20:
  ...
  if ggl == ([],[],[],[],'','', []) and bng == ([],[],[]):
  	no_result = {
    'url': '/',
    'descr': 'No Result (An error may have occurred.)',
